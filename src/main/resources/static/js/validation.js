...
document.addEventListener('DOMContentLoaded', function() {
	...
	form.pass2.addEventListener('focusout', function() {
		...
		if(pw1 == pw2) {
			...
			passResult.innerText = '비밀번호가 일치하지않습니다.';
			passResult.style.color = 'red';
			isPassOk = false;
		}
	});
	...
	//이름 검사
	form.us_name.addEventListener('focusout', function() {
	...
	btnCheckNick.addEventListener('click', function() {
		...
		//별명 중복 요청
		fetch(`/sboard/user/nick/${value}`) //주소 변경됨
			.then(res => res.json())
			.then(data => {
				console.log(data);
				if(data.count > 0) {
	...
	//이메일 검사
	let preventDblclick = false; //이중 클릭 방지를 위한 상태변수
	btnCheckEmail.addEventListener('click', function() {
		...
		emailResult.innerText = '이메일로 인증코드 전송 중입니다.';
	...
	//이메일 코드 전송 버튼 클릭
	btnEmailCode.addEventListener('click', async function() {
	...
	// 휴대폰 중복 체크 
	form.hp.addEventListener('focusout', function(){
